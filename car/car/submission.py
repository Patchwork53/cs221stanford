...
import util, math, random, collections, copy
...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                x = util.colToX(col)
                y = util.rowToY(row)
                distance = dist(x,y,agentX,agentY)
                old_p = self.belief.getProb(row,col)

                #probability of the car actually being at = distance when the sonar says = ObservedDist
                distribution = util.pdf(observedDist, Const.SONAR_STD, distance)   
                new_p = old_p * distribution #little iffy on this
                self.belief.setProb(row,col,new_p)
    ...
    def elapseTime(self):
        ...
        arr2D = collections.defaultdict(lambda:0)
        
        for key in self.transProb.keys():
            oldTile, newTile = key
            arr2D[ newTile ] += self.transProb[key]*self.belief.getProb(oldTile[0], oldTile[1])

        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                self.belief.setProb(row, col, arr2D[(row,col)])
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for key in self.particles.keys():
            ...
            y = util.rowToY(row)
            distance = dist(x,y,agentX,agentY)
            p_count = self.particles[key]

            # old_p = self.belief.getProb(row,col)
            distribution = util.pdf(observedDist, Const.SONAR_STD, distance)   
            # p_count = self.particles[key]
            # print(p_count)
            new_p = p_count * distribution 
            weightDict[key] = new_p

        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            key = util.weightedRandomChoice(weightDict)
    ...
    #   of the form ' <your code>' and call
    ...
    def elapseTime(self):
        ...
        for tile in self.particles: 
            n = self.particles[tile]
            for i in range(n):
                key =  util.weightedRandomChoice(self.transProbDict[tile])
                temp[key]+=1

        self.particles = copy.deepcopy(temp)
         
           
        # self.particles = {}
        # for i in range(self.NUM_PARTICLES):
        #     key = util.weightedRandomChoice(weightDict)
        #     if key not in self.particles.keys():
        #         self.particles[key]=1
        #     else:
        #         self.particles[key]+=1
