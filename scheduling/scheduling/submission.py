...
from sympy import Domain, false, true
...
def create_nqueens_csp(n = 8):
    ...
    for p in variables:
        for q in variables:
            ...
            csp.add_binary_factor(p,q, lambda x,y: x!=y)
            csp.add_binary_factor(p,q, lambda x,y: abs(p-q)!= abs(x-y) )
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            mc_var = None
            lowest_consistent_count = float('inf')
            ...
            for var in self.csp.variables:
                ...
                if consistent_count < lowest_consistent_count:
                    lowest_consistent_count = consistent_count
                    mc_var = var
    ...
    def arc_consistency_check(self, var):
        ...
        while len(Q)!=0:
            ...
            #check if values of neighbor is consistent with atleast 1 value of this v

            for var2 in self.csp.get_neighbor_vars(v):
               
                temp = copy.deepcopy(self.domains[var2])

                for val2 in self.domains[var2]:
                    some_consistent = False

                    if self.csp.unaryFactors[var2]!= None and self.csp.unaryFactors[var2][val2] == 0:
                        temp.remove(val2)
                        continue

                    for val1 in self.domains[v]:
                        if self.csp.binaryFactors[v][var2][val1][val2] != 0:
                            some_consistent = True
                            break

                    if not some_consistent:
                        #val2 wasnt consistent with any value of v
                        temp.remove(val2)

                if len(self.domains[var2]) != len(temp):
        ...
        #python grader.py  1c-1-basic

        #python grader.py  1c-2-hidden
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    full_range = range(0,maxSum+1)
    nxn = []
    
    for i in full_range:
        for j in full_range:
            nxn.append((i,j))
    result = ('sum', name, 'totSum')

    csp.add_variable(result, full_range)
    ...
    for i, X_i in enumerate(variables):
        ...
        csp.add_variable(A_i, nxn)

        def factor(xi_val, ai_val):
            return ai_val[0]+xi_val == ai_val[1]

        csp.add_binary_factor(X_i, A_i, factor)
    
        if i==0:
            csp.add_unary_factor(A_i, lambda val: val[0]==0)
        else:
            def factor(b1, b2):
                return b1[1]==b2[0]
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request, quarter in csp.variables:

            if request.quarters is None:
                continue

            if quarter not in request.quarters:
                csp.add_unary_factor((request, quarter), lambda val: val==None )
    ...
    def add_unit_constraints(self, csp):
        ...
        for request, quarter in csp.variables:  
            for courseId in request.cids:
                if courseId == None:
                    continue
                d[quarter].add(courseId)
                dom = list(range(self.bulletin.courses[courseId].minUnits, self.bulletin.courses[courseId].maxUnits+1))+[0]
                to_add.append(((request,quarter),(courseId,quarter), dom))
                # csp.add_variable((courseId,quarter), dom)



        def iftaken(req_val,new_var_val):
            #req_val is courseID
            #new_var_val is a unit(number)
            if req_val == new_var[0]:
                return new_var_val != 0
            else:
                return new_var_val == 0



        for req,new_var,dom in to_add:

            csp.add_variable(new_var, dom)
            csp.add_binary_factor(req, new_var, iftaken)
                    
        for quarter in d.keys():
          
            toSum = [(cid, quarter) for cid in d[quarter]]
            
            sumVar = get_sum_variable(csp, 'unit_constraint_'+quarter, toSum, self.profile.maxUnits)
            csp.add_unary_factor(sumVar, lambda n: n == 6 )

        # print("variable num: ", csp.numVars)
        # print("num unary: ", len(csp.unaryFactors))
        # print("num bin: ", len(csp.binaryFactors))
        # END_YOUR_CODE

    def add_unit_constraints2(self, csp):
        """
        Add constraint to the CSP to ensure that the total number of units are
        within profile.minUnits/maxUnits, inclusively. The allowed range for
        each course can be obtained from bulletin.courses[cid].minUnits/maxUnits.
